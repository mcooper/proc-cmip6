...
obs_raw <- data.frame(rasterToPoints(stack(list.files('~/maldat/raw/cru_ts_4/afr/', full.names=T, pattern='TMP'))))
...
fwrite(obs, '~/maldat/tmp/obs_tas.csv', row.names=F)

obs <- fread( '~/maldat/tmp/obs_tas.csv')

#########################
# Get list of all models
models <- list.files('~/maldat/raw/CMIP6/', pattern='historical.*tas.nc')
models <- substr(models, 12, nchar(models) - 7)
...
for (model in models){
  ...
  hst <- sspStackToPoints(paste0('~/maldat/raw/CMIP6/historical_', model, '_tas.nc'),
                          'date', 'hst')
  ...
  ssp1 <- sspStackToPoints(paste0('~/maldat/raw/CMIP6/ssp126_', model, '_tas.nc'), 'date', 'ssp1')
  ssp2 <- sspStackToPoints(paste0('~/maldat/raw/CMIP6/ssp245_', model, '_tas.nc'), 'date', 'ssp2')
  ssp3 <- sspStackToPoints(paste0('~/maldat/raw/CMIP6/ssp370_', model, '_tas.nc'), 'date', 'ssp3')
  ssp5 <- sspStackToPoints(paste0('~/maldat/raw/CMIP6/ssp585_', model, '_tas.nc'), 'date', 'ssp5')
